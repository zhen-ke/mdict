...
import { REGEXP_STRIPKEY, log, getExtension } from "./mdict-common";
...
function parse_mdict(file, ext) {
  ...
  function Scanner(buf, len) {
    ...
    var methods = {
      ...
      readUTF16: function (len) {
        return conseq(
          UTF_16LE.decode(new Uint8Array(buf, offset, len)),
          this.forward(len)
        );
      },
      ...
      readText: function () {
        ...
        return conseq(
          _decoder.decode(new Uint8Array(buf, offset, len)),
          this.forward(len + _bpu)
        );
      },
      // Read data to an Uint8Array and decode it to text with specified encoding.
      ...
      readBlock: function (len, expectedBufSize, decryptor) {
        ...
        if (comp_type === 0) {
          ...
          tmp =
            comp_type === 2
              ? pako.inflate(tmp)
              : lzo.decompress(tmp, expectedBufSize, 1308672);
          this.forward(len);
  ...
  //   var xml = parseXml(header_str).querySelector(
  //     "Dictionary, Library_Data"
  //   ).attributes;
  ...
  function read_keyword_index(input, keyword_summary) {
    ...
    for (var i = 0, size; i < keyword_summary.num_blocks; i++) {
      keyword_index[i] = {
        ...
        first_word: conseq(
          scanner.readTextSized(size),
          (size = scanner.readShort())
        ),
        // UNUSED, can be ignored
  ...
  function read_record_summary(input, pos) {
    ...
    record_summary.block_pos =
      pos + record_summary.index_len + record_summary.len;
  ...
  function read_object(input, block, keyinfo) {
    if (input.byteLength > 0) {
      ...
      throw (
        "* OUT OF FILE RANGE * " + keyinfo + " @offset=" + block.comp_offset
      );
    }
  }
    ...
    return startFrom.spread(function (kdx, idx, list) {
      ...
      return appendMore(
        ...
        if (_trail.block === KEY_INDEX.length - 1) {
          if (
            _trail.offset + _trail.pos >=
            KEY_INDEX[_trail.block].num_entries
          ) {
            _trail.exhausted = true;
  ...
  return _slice(pos, 4)
    ...
    .spread(function (record_summary) {
      ...
      return _slice(pos, record_summary.index_len).exec(
        read_record_block,
        record_summary
      );
    })
    .spread(function () {
      log("-- parse done --", file.name);
